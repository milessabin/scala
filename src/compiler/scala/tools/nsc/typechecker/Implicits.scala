  ...
  class ImplicitSearch(val tree: Tree, val pt: Type, val isView: Boolean, val context0: Context, val pos0: Position = NoPosition) extends Typer(context0) with ImplicitsContextErrors {
    ...
    def improves(info1: ImplicitInfo, info2: ImplicitInfo) = {
      ...
      def compute = {
        ...
        if(result0) true
          ...
          val result1 = isStrictlyMoreSpecific(restpe2, restpe1, info2.sym, info1.sym, false)
          if(result1) false
          else {
            ...
            if(imp1 && imp2) {
              ...
              if(params1.length > params2.length) true
                ...
                isStrictlyMoreSpecific(info1.tpe, info2.tpe, info1.sym, info2.sym, false)
            } else imp1
          }
        }
      }
    ...
    class InductiveImplicitComputation(iss: Infoss, isLocalToCallsite: Boolean) extends AbstractImplicitComputation(iss, isLocalToCallsite) {
      ...
      def isIneligibleForInduction(info: ImplicitInfo) = (
           info.sym.hasFlag(LOCKED)
        || info.computeErroneous // avoid side effects
