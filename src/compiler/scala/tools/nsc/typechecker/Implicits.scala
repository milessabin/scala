    ...
    def tryInduction(iss: Infoss): SearchResult = {
      ...
      val inductionResult =
        if(!inductive(local, scope)) nonInductive
          ...
          // easily visible.
          def loop(pt: Type): SearchResult = {
            ranked.foldLeft(nonInductive: SearchResult) {
              case (acc, info) if acc.isFailure =>
          ...
          def applyImplicitArgs(fun: Tree, ftpe: Type, init: SearchResult): Tree = ftpe match {
            case MethodType(params, mt) =>
              ...
              for(param <- params) {
                ...
                val res =
                  ...
                  else inferImplicit(fun, paramTp, reportAmbiguous = context.reportErrors, isView = false, context = context, saveAmbiguousDivergent = true, pos = tree.pos)
      ...
      if (result.isFailure) {
        ...
        if(settings.YinductionHeuristics) {
          if (result.isFailure && !wasAmbiguous) {
            ...
            if (result.isFailure && !result.isNoninductive)
